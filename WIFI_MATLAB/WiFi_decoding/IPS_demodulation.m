...
%%  Input from synchronization
rxSig = frame_sync;  %%frame_sync  synchFrame rxSig WIFI_Waveform  RX_Data
...
ind_Lstf = wlanFieldIndices(cfgVHT,'L-STF');
...
% estimation
stf_part = rxSig(ind_Lstf(1):ind_Lstf(2),:);
...
rxSig(ind_Lltf(1)+32:ind_Lltf(2)) = rxSig(ind_Lltf(1)+32:ind_Lltf(2)) .* exp(-1i * mmC' * phase_STF);

%% Fine CFO Estimation
Ts = 1/(2*10^7);
ltf_part = rxSig(ind_Lltf(1):ind_Lltf(2),:);
...
mmF = 128:128+ind_Data(2)-ind_Lltf(2)-1;
% mmF = 128:ind_Data(2);
...
% rxSig(ind_Lltf(2)+1:end) = rxSig(ind_Lltf(2)+1:end) .* exp(-j * mmF * phase_STF);
% rxSig(ind_Lltf(2)+1:end) = rxSig(ind_Lltf(2)+1:end) .* exp(-j * mmF * phase_LTF);

%% L-LTF FFT
rxSig_lega_ltf = rxSig(ind_Lltf(1):ind_Lltf(2)); 
