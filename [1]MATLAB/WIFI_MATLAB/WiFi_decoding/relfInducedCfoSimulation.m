...
frame300 = repmat(wholeWaveform_scaling,1,frameLength);
for adcfo=1:frameLength
    framewithCfo(:,adcfo) = tgacChan(chNoise(pfOffset(frame300(:,adcfo), (adcfo-1)*100)));
    
    rxSig = framewithCfo(:,adcfo);
    ...
    pilot_remove_acc(:,adcfo) = pilot_remove_;
    
    pilot_remove_cfo =[aft_estcom_cfo(1:7,:);aft_estcom_cfo(9:21,:);aft_estcom_cfo(23:34,:);aft_estcom_cfo(36:48,:);aft_estcom_cfo(50:56,:)];
    pilot_remove_cfo_ =(reshape(pilot_remove,[52*Nsym,1]));
    pilot_remove_cfo_acc(:,adcfo) = pilot_remove_cfo_; 
...
xlim([-1.5 1.5])
ylim([-1.5 1.5])
% pilot_remove_ =(reshape(framewithCfo,[52*Nsym,1]));
sz = 25;
scatter(real(pilot_remove_acc(:,nnnsss)),imag(pilot_remove_acc(:,nnnsss)),sz,'*')
