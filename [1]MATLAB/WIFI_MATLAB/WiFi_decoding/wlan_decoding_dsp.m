...
% rxSig = frame_sync;  %% synchFrame  WIFI_Waveform RX_Data   wholeWaveform_scaling
...
rf_data_combine = reshape(rf_data_,length(rf_data_int16_real)*2,1);
...
rf_data_combine_4frames_offset = [rf_data_combine_4frames(321:end);rf_data_combine_4frames(1:320)];
...
pilot_scale_trans = pilot_scale_sum';

% calculate angle 
real_rcfo = real(pilot_scale_trans);
imag_rcfo = imag(pilot_scale_trans);
% 
% angle_RCFO = (imag_rcfo./real_rcfo);
...
% Applying CFO algorithm
% for estnn = 1:Nsym
%     compensated_real_dsp(:,estnn) = compensated_real(:,estnn).* cos_rcfo(estnn) - compensated_imag(:,estnn).* sin_rcfo(estnn);
%     compensated_imag_dsp(:,estnn) = compensated_real(:,estnn).* sin_rcfo(estnn) + compensated_imag(:,estnn).* cos_rcfo(estnn);
% end

% Not Applying CFO algorithm
...
pilot_remove_one = reshape(pilot_remove,numel(pilot_remove),1);
...
sz = 25;
scatter(real(pilot_remove_test_one),imag(pilot_remove_test_one),sz,'*')
% scatter(real(pilot_remove_one),imag(pilot_remove_one),sz,'*')
xlabel('In-phase')
ylabel('Quadrature')
