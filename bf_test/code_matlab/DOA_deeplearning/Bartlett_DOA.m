% Compute the Bartlett DOA estimate
...
% Output:
%   P_Bartlett:   Spatial spectrum over scanning angles

function [P_Bartlett]=Bartlett_DOA(h_Rxx,M0,lambda,angle_scan)
mu = -2*pi/lambda*lambda/2*sind(angle_scan);
n = 0 : M0-1;
% all steering vector: M0 (# antenna element) x N (# of scan angle)
sv_all = exp(1i*(n.'-(M0-1)/2)*mu);

P_Bartlett = zeros(length(angle_scan),1);

for i = 1:length(angle_scan)
    sv = sv_all(:,i);
    P_Bartlett(i) = abs(sv'*h_Rxx*sv/(sv'*sv));
end
% figure
